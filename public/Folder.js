...
class Folder {
	...
	treeSearch() {
		...
		this.folders.forEach((el, index, arr) => {
			...
			const SubFolder = new Folder(subFolderPath);
	...
	createSubFoldersAndCopy(targetPath) {
		...
		for(const file of files) {
			...
			streamFile(sourceFilePath, targetFilePath);
		}
	...
	async createSubFoldersAndCopyAsync(targetPath) {
		try {
			...
			for(const file of files) {
				...
				console.log('sourceFilePath: ', sourceFilePath);
				console.log('targetFilePath: ', targetFilePath);
				const streamFileResponse = await streamFilePromise(sourceFilePath, targetFilePath);
